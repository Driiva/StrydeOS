...
3. Add your production domain, e.g. `app.strydeos.com` or `yourdomain.com`  
4. Save
...
- **`NEXT_PUBLIC_APP_URL`** = your canonical app URL, e.g. `https://app.strydeos.com`  
  - Used for invite/login links and any client-visible links. Must use HTTPS and the exact domain users see.
- **`APP_URL`** = same value, e.g. `https://app.strydeos.com`  
